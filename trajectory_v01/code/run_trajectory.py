...
def main():
    ...
    paths=[root/'code/run_trajectory.py',root/'code/run.sbatch',root/'src/trajectory_hmm.py',
    ...
    identity=dict(run_id='conditional_trajectory_v01',arm=arm,mode=mode,s=s,df=1800,
    ...
    def one(i):
        ...
        return post,err,oracle_error,ll
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for start in range(0,200,4):
            for i,(post,err,oe,ll) in zip(range(start,start+4),pool.map(one,range(start,start+4))):
